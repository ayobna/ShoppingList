...
import { View, StyleSheet, FlatList, TouchableHighlight, Pressable } from 'react-native';
import { TextInput, IconButton, Button, Text, Avatar } from 'react-native-paper';
...
const CreateListScreen = (props) => {
  ...
  (
    <ProductCard data={itemData.item} handleDeleteProduct={handleDeleteProduct} handleEditProduct={handleEditProduct} />
  );

  // פעולה אשר אחראית על החסרה/הוספה של כמות
  const handlePlusMinusAmount = (operation, edit) => {
    if (amount === "" || (productEditDetails && productEditDetails.Amount === "")) {
      edit ?
  ...
  const handleAddProduct = () => {
    ...
    let product = {
      ...
      Amount: amount,
      Img: imageBase64,
      ImgUri: imageUri ? imageUri : "https://media.istockphoto.com/vectors/no-image-available-sign-vector-id922962354?k=20&m=922962354&s=612x612&w=0&h=f-9tPXlFXtz9vg_-WonCXKCdBuPUevOBkp3DQ-i0xqo=",
    }
  ...
  const regexValidationProduct = (edit) => {
    let counter = 0;
    const amountRgx = /^[1-9]+$/;
    const nameRgx = /^[\u05D0-\u05EAa-zA-Z0-9']+([ |\-][\u05D0-\u05EAa-zA-Z0-9']+){0,1}$/;
  ...
  const handleConfirmEdit = () => {
    ...
    let index = tempProducts.findIndex(product => product.ProductID === productEditDetails.ProductID);
  ...
  const regexValidationShoppingList = () => {
    let counter = 0;
    const titleRgx = /^[\u05D0-\u05EAa-zA-Z0-9']+([ |\-|.|/][\u05D0-\u05EAa-zA-Z0-9']+)*$/;
      ...
      <View style={{ paddingVertical: 5, paddingHorizontal: 5, borderTopWidth: 2, borderColor: "black", backgroundColor: "#b1b1b1" }}>
        ...
        <View style={{ width: "100%", flexDirection: "row", justifyContent: "space-between", marginTop: 5 }}>
          <View style={{ width: "49%", justifyContent: "center", alignItems: "center", flexDirection: "row", justifyContent: "space-around" }}>
            <Text>כמות: </Text>
            <AmountInput edit={false} isError={amountError} amount={amount} handlePlusMinusAmount={handlePlusMinusAmount} handleOnChageAmount={handleOnChageAmount} />
      ...
      </View>
      {
        ...
        >
          <TextInput
            ...
            onChangeText={(txt) => setProductEditDetails(oldstate => ({
              ...oldstate,
              Name: txt,
            }))}
            dense={true}
            ...
            mode="outlined"
          />
          <View style={{ marginTop: 15, justifyContent: "flex-start", alignItems: "center", flexDirection: "row" }}>
            <Text>כמות: </Text>
            <AmountInput isError={editAmountError} edit={true} amount={productEditDetails.Amount} handlePlusMinusAmount={handlePlusMinusAmount} handleOnChageAmount={handleOnChageAmount} />
          </View>
          <View style={styles.editImageContainer}>
            ...
            <View style={styles.editImageIcon}>
              <TouchableHighlight onPress={null} underlayColor='white' style={styles.touchableProfileCameraAvatar}>
                <IconButton
            ...
            <View style={styles.removeImageIcon}>
              <IconButton
                ...
                color='white'
                size={20}
                onPress={() => setProductEditDetails(oldState => ({ ...oldState, ImgUri: "https://media.istockphoto.com/vectors/no-image-available-sign-vector-id922962354?k=20&m=922962354&s=612x612&w=0&h=f-9tPXlFXtz9vg_-WonCXKCdBuPUevOBkp3DQ-i0xqo=", ImageBase64: null }))}
              />
            </View>
          </View>
        </PopupDialog>
